...
from ossac.algorithms.critics.twin_continuous_q_critic import TwinContinuousQCritic
...
class SoftTwinContinuousQCritic(TwinContinuousQCritic):
    ...
    def train(
        ...
        # mento carlo estimation
        # expand_actions = self.expand_action(actions)
        # expand_next_actions = self.expand_action(next_actions)
        # p_q = self.mellow_max(self.critic(torch.tile(share_obs, [num+1, 1]), expand_actions))
        # p_next_q = self.mellow_max(self.target_critic(torch.tile(next_share_obs, [num+1, 1]), expand_next_actions))
