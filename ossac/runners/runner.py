...
from ossac.common.valuenorm import ValueNorm
from torch.distributions import Categorical
from ossac.utils.trans_tools import _t2n
from ossac.utils.envs_tools import (
    make_eval_env,
    make_train_env,
    make_render_env,
    set_seed,
...
from ossac.utils.models_tools import init_device
from ossac.algorithms.ossac import OSSAC as Policy
from ossac.algorithms.critics.soft_twin_continuous_q_critic import SoftTwinContinuousQCritic
from ossac.common.buffers.off_policy_buffer_fp import OffPolicyBufferFP
from ossac.utils.configs_tools import init_dir, save_config, get_task_name
...
class KMeans:
    ...
    def fit(self, X):
        ...
        for i in range(self.max_iter):
            ...
            if torch.norm(new_centroids - centroids, p=2) < self.tol:
                break
...
class Runner:
    def __init__(self, args, algo_args, env_args):
        ...
        params = list(self.actor[0].actor.parameters())
        for i in range(1, len(self.actor)):
            params += list(self.actor[i].actor.parameters())
        self.group_optim = torch.optim.Adam(params, lr=self.algo_args["model"]["lr"])

    # construt group embeddings
    def get_group_embedding_eval(self, agent_order, sp_obs):
        # return sp_obs
        # k=3
        # agent_embeddings = sp_obs.to(self.device)
        
        sp_obs = torch.from_numpy(sp_obs).to(dtype=torch.float32, device=self.device)
        k=3
        agent_embeddings = torch.zeros(0).to(self.device)
        for agent_id in agent_order:
            # construct graph adj
            agent_embedding = self.actor[agent_id].actor.get_group_embedding(sp_obs[:, agent_id]).unsqueeze(1)
            agent_embeddings = torch.cat([agent_embeddings, agent_embedding], dim=1)
        
        # return agent_embeddings.detach()
        group_graph = torch.zeros((agent_embeddings.shape[0], self.num_agents, self.num_agents)).to(self.device)
        for b in range(agent_embeddings.shape[0]):
            clusters_labels = KMeans(n_clusters=k).fit(agent_embeddings[b])  
            for agent_id in agent_order:
                ind = (clusters_labels==clusters_labels[agent_id])
                group_graph[b, agent_id, ind] = 1
        graph_obs = torch.matmul(group_graph, sp_obs)
        return graph_obs
    
    # construt group embeddings
    def get_group_embedding(self, agent_order, sp_obs):
        # return sp_obs
        # k=3
        sp_obs = torch.from_numpy(sp_obs).to(dtype=torch.float32, device=self.device)
        k=3
        ...
        for agent_id in agent_order:
            # construct graph adj
            agent_embedding = self.actor[agent_id].actor.get_group_embedding(sp_obs[agent_id]).unsqueeze(0)
            agent_embeddings = torch.cat([agent_embeddings, agent_embedding], dim=0)
        
        # return agent_embeddings.detach()
        group_graph = torch.zeros((agent_embeddings.shape[1], self.num_agents, self.num_agents)).to(self.device)
        for b in range(agent_embeddings.shape[1]):
            clusters_labels = KMeans(n_clusters=k).fit(agent_embeddings[:, b])
            for agent_id in agent_order:
                ind = (clusters_labels==clusters_labels[agent_id])
                group_graph[b, agent_id, ind] = 1
        graph_obs = torch.matmul(group_graph, sp_obs.permute(1, 0, 2))
        graph_obs = graph_obs.permute(1, 0, 2)
        return graph_obs
    
    def expand_action(self, actions, sp_available_actions, agent_id):
        ...
        return random_actions_tensors_t

    def mellow_max(self, q_vals):
        # q_vals = q_vals.reshape(num+1, -1)
        max_q_vals = q_vals.max(dim=0, keepdim=True)[0]
        q_vals -= max_q_vals
        exp_q = torch.exp(beta * q_vals)
        sum_exp_q = torch.sum(exp_q, dim=0)/(self.num_random+1)
        softmax_q = torch.log(sum_exp_q) + max_q_vals * beta
        return softmax_q
    ...
    def train(self):
        ...
        next_actions = []
        next_logp_actions = []
        graph_obs = self.get_group_embedding(range(self.num_agents), sp_obs)
        for agent_id in range(self.num_agents):
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            graph_obs = self.get_group_embedding(agent_order, sp_obs)
            ...
            for agent_id in agent_order:
                ...
                # potential Q
                random_actions_tensors_t = self.expand_action(actions, sp_available_actions, agent_id)
                ...
                # value_pred = self.mellow_max(value_pred)
    ...
    def get_actions(self, obs, available_actions=None, add_random=True, share_obs=None):
        ...
        """
        actions = []
        graph_obs = self.get_group_embedding_eval(range(self.num_agents), obs)
        for agent_id in range(self.num_agents):
