...
from icmrp.common.valuenorm import ValueNorm
from torch.distributions import Categorical
from icmrp.utils.trans_tools import _t2n
from icmrp.utils.envs_tools import (
    make_eval_env,
    make_train_env,
    make_render_env,
    set_seed,
...
from icmrp.utils.models_tools import init_device
from icmrp.algorithms.icmrp import icmrp as Policy
from icmrp.algorithms.critics.soft_twin_continuous_q_critic import SoftTwinContinuousQCritic
from icmrp.common.buffers.off_policy_buffer_fp import OffPolicyBufferFP
from icmrp.utils.configs_tools import init_dir, save_config, get_task_name
...
class Runner:
    ...
    def get_group_embedding_eval(self, agent_order, sp_obs):
        ...
        for agent_id in agent_order:
            # construct graph adj
            agent_embedding = self.actor[agent_id].actor.get_group_embedding(sp_obs[:, agent_id], sp_obs, sp_obs).unsqueeze(1)
    ...
    def get_group_embedding(self, agent_order, sp_obs):
        ...
        for agent_id in agent_order:
            # construct graph adj
            agent_embedding = self.actor[agent_id].actor.get_group_embedding(sp_obs[agent_id], sp_obs, sp_obs).unsqueeze(0)
    ...
    def mellow_max(self, actions, sp_available_actions, agent_id, sp_share_obs):
        random_actions_tensor = torch.zeros(self.num_random, len(actions), actions[0].shape[0], sp_available_actions.shape[-1]).to(self.device)
        sp_index = torch.FloatTensor(sp_available_actions).to(self.device)
        for i in range(self.num_random):
            for j in range(self.num_agents):
    ...
    def train(self):
        ...
        next_actions = []
        next_logp_actions = []
        graph_obs, ind = self.get_group_embedding(range(self.num_agents), sp_obs)
        for agent_id in range(self.num_agents):
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            for agent_id in agent_order:
                ...
                # potential Q
                # value_pred = self.mellow_max(actions, sp_available_actions, agent_id, sp_share_obs)
                
                
                contrastive_loss = self.actor[agent_id].get_similar(graph_obs, ind, agent_id)
                ...
                actor_loss += beta * contrastive_loss
